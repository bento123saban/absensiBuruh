...
class absensi {
    ...
    async allRequest() {
        ...
        const intv = setInterval(()=> {
            if (!this.threeDaysGood || !this.requestGood) return
            clearInterval(intv)
    ...
    async countCamera() {
        try {
            ...
            this.cams = cams
            this.switchBtn.classList.toggle("dis-none", (cams.length < 2))
        }
        catch (e) {
            console.error("COunt Camera Error : ", e.message)
        }
    }
    async setCamera() {
        STATIC.loaderRun("Setup Camera")
        await this.countCamera()
        let tryInerval = null
        try {
            ...
            const stream = await navigator.mediaDevices.getUserMedia({
                video: { facingMode: {exact : this.cameraID }},
                audio: false
            ...
            let counter = 5,
                erx = ((err == "" || err == "Error") ? "Not Support/Not Found" : err)
            ...
            STATIC.verifyController({
                ...
                tryInerval = setInterval(()=> {
                    ...
                    document.querySelector("#verify span").innerHTML = erx + " (" + counter + ")"
                }, 1000)
            })
            STATIC.loaderStop()
        }
    ...
    async readData(){
        console.log("read")
        return await JSON.parse(localStorage.getItem("dataBuruh"))
    }
    autoDetectLocation() {
    ...
    makeInsecureUUID() {
        ...
        return uuid + " " + new Date().getDate() + "/" + (new Date().getMonth() + 1) + "/" + new Date().getFullYear() + "_" + new Date().getHours() + ":" + new Date().getSeconds() ;
    }
    ...
    setDataPengawas() {
        ...
        this.dataBrx = false
        const dataBuruh = JSON.parse(localStorage.getItem("dataBuruh"))
        if (!dataBuruh) document.querySelector("#to-data").classList.add("dis-none")
        ...
        dataBuruh.pengawas.forEach(man => {
            html += `<option value="${man.NAMA}">${man.NAMA}</option>`
        })
...
//new absensi().autoDetectLocation()
new absensi()



async function testAPI () {
    const response = await fetch("https://api.geoapify.com/v1/geocode/reverse?lat=-3.695486&lon=128.188421&apiKey=3cb0b85652414d5391379c4afd93e4f8", {
        method: 'GET',
    })
...
//console.log(new absensi().makeInsecureUUID())